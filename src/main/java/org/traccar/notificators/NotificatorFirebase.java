...
public class NotificatorFirebase implements Notificator {
    ...
    public NotificatorFirebase(
       ...
       var k = Keys.NOTIFICATOR_FIREBASE_SERVICE_ACCOUNT;

        FileInputStream serviceAccount = new FileInputStream("D:/Projetos/Veiculos/traccar/src/main/resources/google-services.json");
    ...
    @Override
    public void sendDirect(User user, Mensagem mensagem) throws MessageException {
        List<String> registrationTokens = new ArrayList<>(
                Arrays.asList(user.getString("notificationTokens").split("[, ]")));

        MulticastMessage message = MulticastMessage.builder()
                .setNotification(com.google.firebase.messaging.Notification.builder()
                        .setTitle(mensagem.getTitulo())
                        .setBody(mensagem.getConteudo())
                        .build())
                .setAndroidConfig(AndroidConfig.builder()
                        .setNotification(AndroidNotification.builder()
                                .setSound("default")
